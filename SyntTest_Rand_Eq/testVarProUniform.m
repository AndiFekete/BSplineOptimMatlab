...
for Nkn=1:length(sig) %Nkn+5 is the number of knots.
    ...
    prdVarPro{Nkn}=zeros(1,size(knots{Nkn},2));
    ...
    for i=1:1:size(knots{Nkn},2)
        ...
        knots0{Nkn}(:,i)=linspace(1,length(y),Nkn+5);
        ...
        [c,aprxvarpro]=bspline_coeffs(y,knotsVarPro{Nkn}(:,i),4,show);
        prdVarPro{Nkn}(i)=norm(y-aprxvarpro')/norm(y-mean(y))*100;    
    end
end
save('ResultsVarProUniform.mat','knots0','knotsVarPro','prdnullad0','prdnullad3','prdVarPro');
